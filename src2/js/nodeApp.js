...
function addVoteKind(){
	...
	web3.eth.getAccounts(function(error, accounts) {
		...
		App.contracts.NodeManager.deployed().then(function(instance) {
			...
			var rate = $("#voteRate").val();
			var minTime = $("#voteMinTime").val();
			var maxTime = $("#voteMaxTime").val();
			var call = $("#voteCall").val();
  ...
  function openVoteKindList(){
		...
		App.contracts.NodeManager.deployed().then(function(instance){
			...
			for(var i = voteIndex;i > 0;i--){
				nodeContract.getVoteKind.call(i)
				.then(function(data){
					var html = ""
						+"<tr>"
